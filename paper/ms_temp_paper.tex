...
onset_path_113 = manager.data_file('crops_113.npy')



ims = [np.load(onset_path_113), np.load('crop_114.npy'), np.load('crop_115.npy')]

fig = plt.figure(1, (4.,4.), figsize=texfigure.figsize(pytex, 		figure_width_context='textwidth'))
grid = ImageGrid(fig, 111, nrows_ncols=(2,2), axes_pad=0.0, label_mode="1")

for i, anim in enumerate(ims):
	plt.imshow(anim, axes=grid[i], cmap='sdoaia304', origin='lower')
	
onset_fig = manager.save_figure('onset_fig', fig)
onset_fig.placement = 'b'
onset_fig.caption = r'Expanded view of the origin of the jet feature as it forms. Observe the seperate footpoints forming the "inverted Y" shape of the standard model.'	
\end{pycode}
